use dioxus::prelude::*;
use firemage_webui_component_controls::{Modal, Notice};
...
pub fn VmSnapshots(vm: Value, onchanged: EventHandler<()>) -> Element {
    ...
    rsx! {
        ...
        if saving() { crate::save::Save { vm: vm.clone(), onclose: move |_| saving.set(false), onsaved: move |_| { saving.set(false); onchanged.call(()); } } }
...
fn RestorePicker(vm: Value, onclose: EventHandler<()>, onchanged: EventHandler<()>) -> Element {
    ...
    rsx! {
        Modal { title: "Restore snapshot", onclose,
            ...
            if let Some(snapshot) = selected { crate::restore::Restore { key: r#"{text(&snapshot, "id")}"#, snapshot, vm: vm.clone(), onchanged: move |_| { rows.restart(); onchanged.call(()); } } }
        }
    }
}
