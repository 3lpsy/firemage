...
#[tokio::test]
async fn failed_managed_vm_stop_accepts_stale_socket_but_rejects_unverified_live_peer() {
    ...
    for live in [false, true] {
        let socket = directory
            .path()
            .join(if live { "live.sock" } else { "stale.sock" });
        ...
        let row = firemage_queries::insert_vm(
            ...
            &user.id,
            "failed",
            serde_json::json!({"name":"failed"}).to_string(),
            socket.to_str().unwrap().into(),
        )
        .await
        .unwrap();
        let row =
            firemage_queries::set_vm_state(&db, row, "failed", Some("boot failed".into()), None)
                .await
                .unwrap();
        let stopped = runtime.action(&user.id, &row.id, VmAction::Stop).await;
        if live {
            assert!(
                stopped
                    .unwrap_err()
                    .to_string()
                    .contains("socket peer is not the assigned")
            );
            assert!(socket.exists());
        } else {
            assert_eq!(stopped.unwrap().state, VmState::Stopped);
            assert!(!socket.exists());
        }
        drop(listener);
    }
